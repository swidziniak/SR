...
class SimulatorCore:
    ...
    def next_day(self):
        ...
        for x in range(self.number_of_simulation_steps):
            ...
            log['days'].append({'day': self.today.strftime('%Y-%m-%d'), 'productsToExclude': last_day_excluded_products})
        ...
        with open(f'logs/{self.partners_to_read_data_from[0]}_log.json', 'w') as outfile:
            json.dump(log, sort_keys=True, indent=4, separators=(',', ': '), fp=outfile)
